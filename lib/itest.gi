...
InstallGlobalFunction(ITestSimplex,function(G)
	...
	if n>= m then
		...
		for i in iter do
			iter2:=IteratorOfArrangements(m,m);
			for j in iter2 do
				# for lambda in Cartesian( List([1..m], k-> M[i[k]][j[k]]) ) do # for each way of choosing the lambda_k's
				#if Minimum(List(List([1..m], k-> M[i[k]][j[k]]), x->Size(x)))>0 then # fix for iter3 
